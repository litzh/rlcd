// Run: node tests/provision_ui.cjs
...
const listeners={};
const device={name:'RLCD-Test',addEventListener:(n,f)=>listeners[n]=f,gatt:{connected:false,
...
const writer={async writeValueWithResponse(chunk){
  ...
  for(const b of chunk){if(b===10){if(frame.length){const config=JSON.parse(new TextDecoder().decode(Uint8Array.from(frame)));assert.equal(config.ssid,'TestNetwork');assert.equal(config.password,'testpass123');state={...state,state:'connecting',result:'connecting'};}frame=[];}else frame.push(b);}
}};
...
(async()=>{
  ...
  elements.ssid.value='TestNetwork';elements.password.value='short';
  await elements.form.onsubmit({preventDefault(){}});assert.equal(packets.length,0);
  elements.password.value='testpass123';await elements.form.onsubmit({preventDefault(){}});
