...
- CLI 默认读取设备状态并生成序号，通常不传 `--seq`。当前设备只有一个状态位置，新的有效上报会替换当前显示。序号按 Agent/任务独立记录；CLI 自动查询各自序号。
- `--ttl` 默认 120 秒，允许 5–86400。超时 API 返回 `stale`，屏幕回到时钟/传感器待机页。按预期阶段长度选择 TTL，在实际继续工作时更新；不要建立无期限循环或声称已启动后台心跳。
...
以上序号仅示意。发送前通过 `GET /agent/state?agent_id=...&task_id=...` 读取该任务的序号，再使用更大的整数序号（1–4294967295），不要固定使用示例值。重复或旧序号返回 409，且不会刷新 TTL。设备重启后不保留任务序号。
...
`rlcd send working --agent deepseek --task task-42 --pet deepseek-whale --title '正在思考'` 可在上报时临时激活已安装的宠物。HTTP 字段是 `pet_id`。每次需要激活自己的宠物时都带上这个字段；省略会保持其他客户端最后选中的宠物。不存在的宠物会使整条请求失败，不消费序号。上报不自动安装素材，不改变 `pet use` 保存的默认选择。
